document.addEventListener('DOMContentLoaded', function () {
    const sellerHeaderShell = document.querySelector('[data-seller-header-shell]');

    if (!sellerHeaderShell) {
        return;
    }

    const sellerSuggestionsUrl = sellerHeaderShell.dataset.sellerSuggestionsUrl || '';
    const sellerNotificationUserId = Number(sellerHeaderShell.dataset.sellerNotificationUserId || 0);
    const sellerNotificationFeedUrl = sellerHeaderShell.dataset.sellerNotificationFeedUrl || '';
    const sellerNotificationBaseUrl = sellerHeaderShell.dataset.sellerNotificationBaseUrl || '';
    ...
    if (searchInput && searchClearButton && suggestionsBox && sellerSuggestionsUrl) {
        ...
        searchInput.addEventListener('input', function () {
            ...
            fetch(`${sellerSuggestionsUrl}?q=${encodeURIComponent(query)}`, {
    ...
    let sellerUnreadCount = Number(sellerHeaderShell.dataset.sellerUnreadCount || 0);
    ...
    const updateSellerNotificationBadge = (count) => {
        ...
        sellerHeaderShell.dataset.sellerUnreadCount = String(sanitizedCount);
    ...
    const openSellerChatFromNotification = async (link) => {
        ...
        const readForm = notificationItem?.querySelector('[data-seller-notification-read-form]');
    ...
    if (sellerNotificationMenu) {
        ...
        document.addEventListener('seller:notification-received', function () {

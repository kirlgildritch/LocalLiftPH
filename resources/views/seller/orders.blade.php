        ...
        <div class="dashboard-layout">
            ...
            <main class="dashboard-main">
                <section class="seller-page-panel panel">
                    ...
                    @if($errors->any())
                        <p class="seller-feedback error-message">{{ $errors->first() }}</p>
                    @endif
                                                ...
                                                <form method="POST" action="{{ route('seller.orders.shipping-status', $order) }}" style="display: flex; gap: 8px; align-items: center; flex-wrap: wrap;">
                                                    ...
                                                    <select name="shipping_status" class="table-select">
                                                        @foreach($nextStatuses as $status)
                                                            <option value="{{ $status }}">{{ \App\Models\Order::progressStatuses()[$status]['label'] ?? ucfirst(str_replace('_', ' ', $status)) }}</option>
                                                        @endforeach
                                                    </select>
